...
import { X, Share2, Users, Check, List } from 'lucide-react';
...
const BulkShareModal: React.FC<BulkShareModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#212121] rounded-lg shadow-xl max-w-lg w-full border dark:border-[#404040]">
        {/* Header */}
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-500/50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              <List className="text-green-600 dark:text-green-400" size={16} />
              <h3 className="font-semibold text-green-900 dark:text-green-300">
                Wybrane zadania ({selectedTasks.length})
              </h3>
            </div>
            <div className="max-h-32 overflow-y-auto space-y-1">
              {selectedTasks.map((task, index) => (
                <div key={task._id} className="text-sm text-green-800 dark:text-green-400">
                  {index + 1}. {task.title}
                </div>
              ))}
            </div>
                ...
                filteredUsers.map((user) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-[#2a2a2a] cursor-pointer border-b border-gray-100 dark:border-[#404040] last:border-b-0 ${
                      selectedUsers.includes(user.id) ? 'bg-green-50 dark:bg-green-900/20' : ''
                    }`}
        ...
        <div className="flex gap-3 p-6 border-t border-gray-200 dark:border-[#404040]">
          ...
          <button
            ...
            className="btn btn-primary flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700"
          >
            {isLoading ? (
              <>
